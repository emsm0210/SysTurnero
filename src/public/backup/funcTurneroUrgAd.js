    ...
    $.post(URL + '/atendidosTurnero/1/8')
        .done(function(data) {
            ...
            if (json.length >= 4) {
                for (let x in json) {
                    tabla += '<tr id="fila_' + x + '">';
                    tabla += '<td style="width: 10%;" class="red">' + json[x].consultorio.toString().replace('80', '') + '</td>';
    ...
    if (evt.turnero = 'turnero8') {
        ...
        if (evt.atendidos.length >= 4) {
            for (let x in evt.atendidos) {
                tabla += '<tr id="fila_' + x + '">';
                tabla += '<td style="width: 10%;" class="red">' + evt.atendidos[x].consultorio.toString().replace('80', '') + '</td>';
